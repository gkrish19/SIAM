...
def interconnect_estimation():
	...
	network_type, arch_type = obtain_interconnect_spec()
	num_layers, num_tiles_per_layer, ip_activation_per_tile, volume_per_tile = create_injection_rate(network_type, arch_type)
	latency_array = interconnect_latency_estimation()
	interconnect_latency = postprocess_latency_array(num_layers, num_tiles_per_layer, ip_activation_per_tile, volume_per_tile, latency_array, network_type, arch_type)
	...
	power_file = open('Final_Results/Energy.csv', 'a')
	power_file.write('Total interconnect energy is, ' + str(float(interconnect_power)*float(interconnect_latency)) +  ', pJ')
	power_file.close()

	latency_file = open('Final_Results/Latency.csv', 'a')
...
def obtain_interconnect_spec():

	spec_file = 'interconnect_spec_file.txt'
	arch_type =os.popen('grep "arch_type" ' + spec_file + ' | tail -1 | awk \'{print $3}\'').read().strip()
	network_type =os.popen('grep "network_type" ' + spec_file + ' | tail -1 | awk \'{print $3}\'').read().strip()
...
def interconnect_latency_estimation():
	...
	for file in files :
		...
		if (mesh_size < 30):
			# Open read file handle of config file
			fp = open('./mesh_config_inj_rate', 'r')
	...
	outfile = open('./logs/latency_mesh.csv', 'w')
...
def create_injection_rate(network_type, arch_type):


	injection_directory_name = 'inj_dir'
	...
	num_tiles_per_layer = pd.read_csv('./to_interconnect/num_tiles_per_layer.csv', header=None)
	ip_activation = pd.read_csv('./to_interconnect/ip_activation.csv', header=None)
	fps = pd.read_csv('./to_interconnect/fps.csv', header=None)
	...
	for layer_idx in range(num_layers-1):
		volume_per_tile[layer_idx] = ((ip_activation.loc[layer_idx+1][0]*quantization_bit + bus_width)*fps.loc[0][0])/ \
		(num_tiles_per_layer.loc[layer_idx][0]*num_tiles_per_layer.loc[layer_idx+1][0]);

	ip_activation_per_tile = np.zeros(num_layers-1);
	for layer_idx in range(num_layers-1):
		ip_activation_per_tile[layer_idx] = ip_activation.loc[layer_idx+1][0]/(num_tiles_per_layer.loc[layer_idx][0]*num_tiles_per_layer.loc[layer_idx+1][0]);
...
def postprocess_latency_array(num_layers, num_tiles_per_layer, ip_activation_per_tile, volume_per_tile, latency_array, network_type, arch_type):
	...
	for layer_idx in range(0, num_layers-1):
		...
		per_layer_latency[layer_idx] = (effective_delay[layer_idx]+1)*(ip_activation_per_tile[layer_idx]*quantization_bit + 32)/bus_width + avg_const_delay[layer_idx]
