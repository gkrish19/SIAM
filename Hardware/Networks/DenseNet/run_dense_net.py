...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--eff_decay', '-ef', type=float, default=5e-3, metavar='',
        help='Effciency decay for optimizer (default: %(default)s)')
    parser.add_argument(
        '--eff_factor', '-ef_fact', type=float, default=0, metavar='',
        help='Effciency Factor for optimizer (default: %(default)s)')

    parser.add_argument(
        '--beta', '-beta', type=float, default=1.0, metavar='',
        help='Pruning ratio for the model (default: %(default)s)')
    ...
    if not args.train and not args.test and not args.prune:
    ...
    if args.prune:
        #model.load_model()
        print("Pruning the model based on effciency")
        model.prune_retrain(train_params)
        print("Completed Training the pruned model")
        model.load_model_prune()
        print("Data provider test images: ", data_provider.test.num_examples)
        print("Testing...")
        loss, accuracy = model.test(data_provider.test, batch_size=200)
        print("mean cross_entropy: %f, mean accuracy: %f" % (loss, accuracy))
