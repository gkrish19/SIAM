...
if __name__ == '__main__':
	...
	W_conv1_1 = tf.get_variable('conv1_1', shape=[3, 3, 3, 3], initializer=tf.contrib.keras.initializers.he_normal())
	_weights.append(W_conv1_1)
	b_conv1_1 = bias_variable([3])
	output  = tf.nn.relu( batch_norm(conv2d(x,W_conv1_1) + b_conv1_1))

	_input_array.append(tf.transpose(output, (0,3,1,2)))
	W_conv1_2 = tf.get_variable('conv1_2', shape=[3, 3, 3, 3], initializer=tf.contrib.keras.initializers.he_normal())
	_weights.append(W_conv1_2)
	b_conv1_2 = bias_variable([3])
	output  = tf.nn.relu( batch_norm(conv2d(output,W_conv1_2) + b_conv1_2))
	output  = max_pool(output, 2, 2, "pool1")

	_input_array.append(tf.transpose(output, (0,3,1,2)))
	W_conv2_1 = tf.get_variable('conv2_1', shape=[3, 3, 3, 6], initializer=tf.contrib.keras.initializers.he_normal())
	_weights.append(W_conv2_1)
	b_conv2_1 = bias_variable([6])
	output  = tf.nn.relu( batch_norm(conv2d(output,W_conv2_1) + b_conv2_1))

	_input_array.append(tf.transpose(output, (0,3,1,2)))
	W_conv2_2 = tf.get_variable('conv2_2', shape=[3, 3, 6, 6], initializer=tf.contrib.keras.initializers.he_normal())
	...
	_input_array.append(tf.transpose(output, (0,3,1,2)))
	W_conv3_1 = tf.get_variable('conv3_1', shape=[3, 3, 6, 12], initializer=tf.contrib.keras.initializers.he_normal())
	_weights.append(W_conv3_1)
	b_conv3_1 = bias_variable([12])
	output  = tf.nn.relu( batch_norm(conv2d(output,W_conv3_1) + b_conv3_1))

	_input_array.append(tf.transpose(output, (0,3,1,2)))
	W_conv3_2 = tf.get_variable('conv3_2', shape=[3, 3, 12, 12], initializer=tf.contrib.keras.initializers.he_normal())
	_weights.append(W_conv3_2)
	b_conv3_2 = bias_variable([12])
	output  = tf.nn.relu( batch_norm(conv2d(output,W_conv3_2) + b_conv3_2))

	_input_array.append(tf.transpose(output, (0,3,1,2)))
	W_conv3_3 = tf.get_variable('conv3_3', shape=[3, 3, 12, 12], initializer=tf.contrib.keras.initializers.he_normal())
	_weights.append(W_conv3_3)
	b_conv3_3 = bias_variable([12])
	...
	_input_array.append(tf.transpose(output, (0,3,1,2)))
	W_conv4_1 = tf.get_variable('conv4_1', shape=[3, 3, 12, 25], initializer=tf.contrib.keras.initializers.he_normal())
	_weights.append(W_conv4_1)
	b_conv4_1 = bias_variable([25])
	output  = tf.nn.relu( batch_norm(conv2d(output,W_conv4_1) + b_conv4_1))

	_input_array.append(tf.transpose(output, (0,3,1,2)))
	W_conv4_2 = tf.get_variable('conv4_2', shape=[3, 3, 25, 25], initializer=tf.contrib.keras.initializers.he_normal())
	_weights.append(W_conv4_2)
	b_conv4_2 = bias_variable([25])
	output  = tf.nn.relu( batch_norm(conv2d(output,W_conv4_2) + b_conv4_2))

	_input_array.append(tf.transpose(output, (0,3,1,2)))
	W_conv4_3 = tf.get_variable('conv4_3', shape=[3, 3, 25, 25], initializer=tf.contrib.keras.initializers.he_normal())
	_weights.append(W_conv4_3)
	b_conv4_3 = bias_variable([25])
	...
	_input_array.append(tf.transpose(output, (0,3,1,2)))
	W_conv5_1 = tf.get_variable('conv5_1', shape=[3, 3, 25, 25], initializer=tf.contrib.keras.initializers.he_normal())
	_weights.append(W_conv5_1)
	b_conv5_1 = bias_variable([25])
	output  = tf.nn.relu( batch_norm(conv2d(output,W_conv5_1) + b_conv5_1))

	_input_array.append(tf.transpose(output, (0,3,1,2)))
	W_conv5_2 = tf.get_variable('conv5_2', shape=[3, 3, 25, 25], initializer=tf.contrib.keras.initializers.he_normal())
	_weights.append(W_conv5_2)
	b_conv5_2 = bias_variable([25])
	output  = tf.nn.relu( batch_norm(conv2d(output,W_conv5_2) + b_conv5_2))

	_input_array.append(tf.transpose(output, (0,3,1,2)))
	W_conv5_3 = tf.get_variable('conv5_3', shape=[3, 3, 25, 25], initializer=tf.contrib.keras.initializers.he_normal())
	_weights.append(W_conv5_3)
	b_conv5_3 = bias_variable([25])
	...
	output = tf.reshape(output,[-1,2*2*25])

	_input_array.append(output)
	W_fc1 = tf.get_variable('fc1', shape=[100,200], initializer=tf.contrib.keras.initializers.he_normal())
	...
	_input_array.append(output)
	W_fc2 = tf.get_variable('fc7', shape=[200,200], initializer=tf.contrib.keras.initializers.he_normal())
	...
	_input_array.append(output)
	W_fc3 = tf.get_variable('fc3', shape=[200,10], initializer=tf.contrib.keras.initializers.he_normal())
